...
export default function TasksPage() {
  ...
  const columns = useMemo(() => {
    if (groupBy === 'team' && teams) {
      return teams.map(team => ({
        id: team.id,
        title: team.name,
        tasks: filteredAndSortedTasks.filter(t => t.teamId === team.id)
      }));
    }
    return [
      { id: 'pending', title: 'Pending', tasks: filteredAndSortedTasks.filter((t) => t.status === 'Pending') },
      { id: 'in-progress', title: 'In Progress', tasks: filteredAndSortedTasks.filter((t) => t.status === 'In Progress') },
      { id: 'completed', title: 'Completed', tasks: filteredAndSortedTasks.filter((t) => t.status === 'Completed') },
    ];
  }, [filteredAndSortedTasks, groupBy, teams]);
                ...
                <div className="p-4 space-y-4 overflow-y-auto">
                  {column.tasks.map(task => (
                    <TaskCard
                      ...
                      teamName={teams?.find(t => t.id === task.teamId)?.name}
                    />
                  ))}
